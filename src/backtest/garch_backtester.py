...
class GarchBacktester:
    ...
    def load_data(self, csv_path):
        ...
        returns = self.df['log_ret'].values
        
        # 2. Parameters (Standard GARCH(1,1))
        # Omega, Alpha, Beta
        # For 1m data, variance is tiny. 
        # We use a standard set or pre-estimated.
        # Let's use EWMA-like params which are robust: Alpha=0.04, Beta=0.94 (RiskMetrics-ish)
        omega = 1e-8 # Small constant
        alpha = 0.05
        beta = 0.90
        
        # 3. Recursive Calculation (Numba would be faster, but loop is okay for 500k in python ~1-2s)
        n = len(returns)
        sigma2 = np.zeros(n)
        sigma2[0] = np.var(returns) # Init
        
        # Vectorized is hard for recursion, use loop
        # (For speed in backtest, simple loop is fine)
        for t in range(1, n):
            sigma2[t] = omega + alpha * (returns[t-1]**2) + beta * sigma2[t-1]
            
        self.df['garch_vol'] = np.sqrt(sigma2)
        
        # RSI (Keep for trend bias)
        ...
        tr_s = tr.rolling(window=adx_window).mean()
        ...
        self.df['adx'] = dx.rolling(window=adx_window).mean()
    ...
    def rebalance_grid(self, center_price, total_equity, row_data=None):
        ...
        # --- SMART GRID LOGIC (GARCH) ---
        smart_conf = self.config.get('smart_grid', {})
        if smart_conf.get('enabled', False) and row_data is not None:
            # 1. Volatility (GARCH)
            garch_vol = row_data.get('garch_vol', 0)
            if garch_vol > 0:
                vol_scale = smart_conf.get('volatility_scale', 1.0)

                # --- NEW: Trend Adaptive Spacing (Match Live Bot) ---
                trend_scale = 1.0
                adx = row_data.get('adx', 0)
                if adx > 40:
                    trend_scale = 1.0 + min((adx - 40) / 40, 0.5) # Max 1.5x
                
                vol_scale *= trend_scale
                # ----------------------------------------------------

                # DYNAMIC SAFETY FACTOR (Match Live Bot)
                safety_factor = 4.0
                if adx < 30:
                    safety_factor = 2.5
                elif adx < 60:
                ...
                daily_vol = garch_vol * 38
                min_span = daily_vol * safety_factor * vol_scale
                
                # --- OVERRIDE LOGIC (Aggressive Mode) ---
                aggressive = self.config.get('aggressive_mode', False)
                
                # Debug print
                if len(self.orders) == 0:
                     print(f"DEBUG: Aggressive={aggressive}, ADX={adx:.1f}, MinSpan={min_span:.4f}, ConfigSpan={span:.4f}")

                if not aggressive:
                    if min_span > span:
                        span = min_span
                # Else: Trust manual span (0.6% or whatever)
